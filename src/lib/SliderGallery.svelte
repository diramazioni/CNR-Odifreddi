<script>
  ...
  const getThumbnails = (/** @type {{ (array: any): void; (array: any): void; (arg0: Element[]): any; }} */ fn) => {
...
<div id="gallery">
    ...
    />
    <button on:click="{selectImage}" data-img="{currentIndex-1}" id="previous"
      >&#10094;</button>
    <button on:click="{selectImage}" data-img="{currentIndex+1}" id="next"
      >&#10095;</button>
    <!-- <figcaption>{currentImage.text || ""}</figcaption> -->
  
  <!-- <div tabindex="1" role="button" on:click="{selectImage}" id="thumbnails">
